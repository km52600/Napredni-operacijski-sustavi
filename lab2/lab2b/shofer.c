	...
	size_t count, loff_t *f_pos)
{
    ...
    ssize_t count2 = kfifo_from_user(fifo, ubuf, count, &copied);
...
static long control_ioctl(struct file *filp, unsigned int request, unsigned long arg)
{
    ...
    if (_IOC_TYPE(request) != SHOFER_IOCTL_TYPE || _IOC_NR(request) != SHOFER_IOCTL_NR) {
        klog(KERN_WARNING, "Invalid IOC type or nr");
        return -EINVAL;
    }

    if (_IOC_SIZE(request) != sizeof(struct shofer_ioctl)) {
        klog(KERN_WARNING, "Argument size mismatch");
        return -EINVAL;
    }

    retval = copy_from_user(&cmd, (const void __user *)arg, sizeof(struct shofer_ioctl));
    if (retval) {
        klog(KERN_WARNING, "Failed to copy from user space");
        return retval;
    }

    if (cmd.count == 0) {
        klog(KERN_WARNING, "Invalid count (zero)");
        return -EINVAL;
    }
    ...
    // Lock za obraditi FIFO buffere
    spin_lock(&in_buff->key);
    spin_lock(&out_buff->key);

    dump_buffer("Start: in_buff", in_buff);
    dump_buffer("Start: out_buff", out_buff);

    // Premještanje podataka iz in_buff u out_buff
    int i = 0;
    while (i < cmd.count) {
        if (kfifo_len(fifo_in) > 0 && kfifo_avail(fifo_out) > 0) {
            got = kfifo_get(fifo_in, &c);
            if (got > 0 && kfifo_put(fifo_out, c)) {
                LOG("Moved '%c' from input to output", c);
                i++;
            } else {
                klog(KERN_WARNING, "Error in kfifo_get or kfifo_put");
            }
        } else {
            LOG("No data available in input buffer");
            break;
        }
    }

    dump_buffer("End: in_buff", in_buff);
    dump_buffer("End: out_buff", out_buff);

    // Otključavanje FIFO buffera
    spin_unlock(&in_buff->key);
    spin_unlock(&out_buff->key);
